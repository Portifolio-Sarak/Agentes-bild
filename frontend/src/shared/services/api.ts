...
const API_BASE_URL = (import.meta.env as any).VITE_API_URL || 'http://127.0.0.1:8001';
...
export interface ApiKeyStatus {
  ...
  models_status: Array<{
    ...
    elo_rating?: number; // Pontuação de performance (Chatbot Arena)
    performance_score?: number; // Pontuação de performance (Hugging Face)
    win_rate?: number; // Taxa de vitória (Hugging Face)
    total_votes?: number; // Total de votos (Hugging Face)
    organization?: string; // Criador do modelo
    license_type?: string; // Tipo de licença
