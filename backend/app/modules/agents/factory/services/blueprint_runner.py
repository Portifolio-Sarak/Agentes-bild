...
from app.modules.agents.factory.services.agent_db_service import AgentDBService
from app.modules.agents.factory.services.llm_provider import LLMProvider 
from app.modules.agents.mcp.services.runtime_manager import MCPRuntimeManager
...
class BlueprintRunner:
    ...
    async def _execute_logic(self, node: Dict[str, Any], context: Dict[str, Any]):
        ...
        result = False
        try:
            result = self._safe_eval(condition_str)
        except Exception as e:
    ...
    def _safe_eval(self, expr: str) -> bool:
        """
        Avalia uma expressão booleana simples de forma segura usando AST.
        Suporta: ==, !=, >, <, >=, <=, and, or, not, True, False, números e strings.
        """
        operators = {
            ast.Eq: operator.eq,
        ...
        def _eval(node):
            if isinstance(node, ast.Expression):
                ...
                # Para operações matemáticas se necessário no futuro
                pass
                ...
                raise ValueError(f"Variável não permitida na expressão: {node.id}")
            raise TypeError(f"Expressão não suportada: {type(node)}")

        try:
            tree = ast.parse(expr, mode='eval')
            return _eval(tree)
        except Exception as e:
            logger.error(f"Falha no safe_eval de '{expr}': {e}")
            return False
