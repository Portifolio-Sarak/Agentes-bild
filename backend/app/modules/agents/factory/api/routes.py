...
async def get_available_models(current_user = Depends(get_current_user), db: Session = Depends(get_db)):
...
def create_agent(agent_in: schemas.AgentCreate, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    """Cria um novo agente vinculado ao usuário atual"""
    user = current_user
...
def list_agents(current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    """Lista apenas os agentes do usuário atual"""
    return db.query(Agent).filter(Agent.user_id == current_user.id).all()


@router.delete("/{agent_id}")
def delete_agent(agent_id: UUID, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    """
...
def save_agent_blueprint(agent_id: UUID, blueprint: schemas.Blueprint, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
...
def load_agent_blueprint(agent_id: UUID, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def run_agent_blueprint(agent_id: UUID, input_data: Dict[str, Any] = {}, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
...
def create_session(agent_id: UUID, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    agent = db.query(Agent).filter(Agent.id == agent_id, Agent.user_id == current_user.id).first()
    if not agent:
        raise HTTPException(status_code=404, detail="Agente não encontrado")
...
async def chat_with_agent(
    ...
    session = db.query(AgentSession).filter(AgentSession.id == session_id, AgentSession.user_id == current_user.id).first()
    if not session:
...
async def get_session(session_id: UUID, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    session = db.query(AgentSession).filter(AgentSession.id == session_id, AgentSession.user_id == current_user.id).first()
    if not session:
        raise HTTPException(status_code=404, detail="Sessão não encontrada")
...
def get_messages(session_id: UUID, current_user = Depends(get_current_user), db: Session = Depends(get_db)):
    session = db.query(AgentSession).filter(AgentSession.id == session_id, AgentSession.user_id == current_user.id).first()
    if not session:
        raise HTTPException(status_code=404, detail="Sessão não encontrada")
